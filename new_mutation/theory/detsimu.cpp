...
double Detsimu::calculate_det_trajectory(const int sep, 
  ...
  // criteria for stopping calculation
  ...
  {
    // calculate a right eigenvector during allele A being rare 
    ...
    if(eigensolver.eigenvalues()(max_index).real() > 0.0){
      ...
      {
        // interpolate allele frequencies for bar{p} < 1e-6
      ...
      while(change > convergence){
        // while frequency change per step is larger than the convergence criteria
        ...
        {
          // interpolate allele frequencies for ave_p_parent <= bar{p} < ave_p
          double ave_p_parent = 0.0;
          double ave_p = 0.0;
        ...
        if(rep_step % 10000 == 0){
          ...
          if(minute > 10.0){
            // if calculation time exceeds 10 min, return error.
            return(-1.0);
          }
        }
      }
...

  {
    ...
    if(eigensolver.eigenvalues()(max_index).real() > 0.0){
      ...
      {
        // interpolate allele frequencies for 1-bar{p} < 1e-6
      ...
      while(change > convergence){
        ...
        {
          // interpolate allele frequencies for ave_q_parent <= 1-bar{p} < ave_q
          double ave_q_parent = 0.0;
          double ave_q = 0.0;
  ...
  if(std::floor(fin2 * sep) - std::floor(fin1 * sep) && fin2 - fin1 < 1e-6 &&
    ...
    // interpolate frequencies between the endpoints of the two trajectories
  ...
  // should be 1.0 / sep if the two trajectories reach the same point
  return(equ2 - equ1);
}