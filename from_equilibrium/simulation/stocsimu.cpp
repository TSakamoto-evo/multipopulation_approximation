...
Stocsimu::Stocsimu(const Parameter input_para, 
  const std::vector<double>& input_ini_freq, const int input_rep){

  para = input_para;
  ini_freq = input_ini_freq;
  rep = input_rep;
  ...
  for(int i = 0; i < pop_num; i++){
    std::binomial_distribution<> det_num(para.pop_sizes.at(i), ini_freq.at(i));
    allele_num.at(i) = det_num(mt);
  }
}
